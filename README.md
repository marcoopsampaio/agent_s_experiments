...
In this section we provide some comments on how we've set up Agent-S. Note that we have hacked agent s2 only.
...
## Main hacks
Here we describe the main base hacks we inmplemented initially [here](https://github.com/marcoopsampaio/agent_s_experiments/commit/9673002aac525ce0d76f3481f6198569a3f51c5f) which are always active in this hacked version. They are (see also commit message):
- Prevent mouse from ever moving to (0,0) coordinates to avoid the agent crashing while executing due to pyautogui failsafe that stops execution when that happens.
...
## Added options
In addition, support was added to control better what is used or not used when running agent s2 through the cli app. In particular:
- `--empty_knowledge_base`: this flag allows to empty the contents of the knowledge base. This can be used to avoid the agent getting confused from tasks that are somewhat semantically related but not exactly within the target domain. This still supports adding new entries to the knowledge base during execution.
- `--no_knowledge_base_updates`: this flag prevents new entries to be added to the knowledge base.
- `--no_subtask_experience`: this completely turns off searching the knowledge base for related subtasks previous experience.
